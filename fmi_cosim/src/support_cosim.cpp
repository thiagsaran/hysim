...
static char* getTmpPath() {
	char tmplate[13];  // Lenght of "fmuTmpXXXXXX" + null
	sprintf(tmplate, "%s", "fmuTmpXXXXXX");
	//char *tmp = mkdtemp(strdup("fmuTmpXXXXXX"));
	char *tmp = mkdtemp(tmplate);
	if (tmp == NULL) {
...
static void* getAdr(int* s, FMU *fmu, const char* functionName) {
	char name[BUFSIZE];
	void* fp;
	sprintf(name, "%s_%s", getModelIdentifier(fmu->modelDescription),
			functionName);
#ifdef _MSC_VER
...
#endif 
		printf(
				"If some symbols are found, but not others, check LD_LIBRARY_PATH or DYLD_LIBRARYPATH\n");
...
static int loadDll(const char* dllPath, FMU *fmu) {
	...
	if (s == 0) {
		...
		if (s == 1)
			printf("  using fmiGetModelTypesPlatform instead\n");
	}
	fmu->instantiateSlave = (fInstantiateSlave) getAdr(&s, fmu,
			"fmiInstantiateSlave");
	fmu->initializeSlave = (fInitializeSlave) getAdr(&s, fmu,
			"fmiInitializeSlave");
	fmu->terminateSlave = (fTerminateSlave) getAdr(&s, fmu,
			"fmiTerminateSlave");
	fmu->resetSlave = (fResetSlave) getAdr(&s, fmu, "fmiResetSlave");
	fmu->freeSlaveInstance = (fFreeSlaveInstance) getAdr(&s, fmu,
			"fmiFreeSlaveInstance");
	fmu->setRealInputDerivatives = (fSetRealInputDerivatives) getAdr(&s, fmu,
			"fmiSetRealInputDerivatives");
	fmu->getRealOutputDerivatives = (fGetRealOutputDerivatives) getAdr(&s, fmu,
	...
	fmu->getIntegerStatus = (fGetIntegerStatus) getAdr(&x, fmu,
			"fmiGetIntegerStatus");
	fmu->getBooleanStatus = (fGetBooleanStatus) getAdr(&x, fmu,
			"fmiGetBooleanStatus");
	...
	fmu->setDebugLogging = (fSetDebugLogging) getAdr(&s, fmu,
			"fmiSetDebugLogging");
...
static void printModelDescription(ModelDescription* md) {
	...
	if (!md->cosimulation) {
		printf(
				"error: No Implementation element found in model description. This FMU is not for Co-Simulation.\n");
		exit(EXIT_FAILURE);
	}
		...
		if (header) {
			...
			case elm_Real:
				...
				if (separator == ',')
					...
					fprintf(file, "%c%.16g", separator, r);
				}
				break;
			case elm_Integer:
			...
			case elm_String:
				...
				break;
			default:
				fprintf(file, "%cNoValueForType=%d", separator,
						sv->typeSpec->type);
			}
		}
	} // for
...
static void replaceRefsInMessage(const char* msg, char* buffer, int nBuffer,
		FMU* fmu) {
...
void fmuLogger(fmiComponent c, fmiString instanceName, fmiStatus status,
	...
	if (!category)
		category = "?";
	printf("%s %s (%s): %s\n", fmiStatusToString(status), instanceName,
			category, msg);
}
...
void parseArguments(int argc, char *argv[], char** fmuFileName, double* tEnd,
	...
	if (argc > 4) {
		if (sscanf(argv[4], "%d", loggingOn) != 1 || *loggingOn < 0
				|| *loggingOn > 1) {
...
void printHelp(const char* fmusim) {
	printf(
			"command syntax: %s <model.fmu> <tEnd> <h> <loggingOn> <csv separator>\n",
			fmusim);
	printf(
	...
	printf(
			"   <csv separator>. separator in csv file,   optional, c for ';', s for';', defaults to c\n");
}
